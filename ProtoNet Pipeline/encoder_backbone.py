...
def load_encoder(encoder_path, device="cuda"):
    ...
    model.patch_embed.load_state_dict(ckpt["patch_embed"])
    model.cls_token.data.copy_(ckpt["cls_token"])
    model.pos_embed.data.copy_(ckpt["pos_embed"])
    model.encoder_blocks.load_state_dict(ckpt["encoder_blocks"])
    model.enc_norm.load_state_dict(ckpt["enc_norm"])
